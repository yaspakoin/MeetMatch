...
import { Send, Clock, ArrowLeft, Lock, Sparkles, MoreVertical, AlertTriangle, ShieldOff, BrainCircuit } from 'lucide-react';
import { generateDeepMatchAnalysis } from '../services/geminiService';
...
export const Chat: React.FC<ChatProps> = ({ match, onBack }) => {
  ...
  useEffect(() => {
    ...
    // If matchReasoning exists (from Dashboard generation), use it. 
    // Fallback to generating one on the fly if missing (e.g. older matches)
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    autoReplyTimeoutRef.current = setTimeout(() => {
      const reply: ChatMessage = {
        ...
        senderId: match.id,
        text: "Essa é uma perspetiva interessante... não estava à espera disso.",
        timestamp: new Date()
      };
    ...
    <div className="h-screen flex flex-col bg-gray-50 relative">
      ...
      <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <img 
              src={match.avatarUrl} 
              className="w-10 h-10 rounded-full object-cover blur-[2px]" // Keeping slight blur for "Anonymous" vibe
              alt={`Avatar de ${match.name}`}
            />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-green-500 font-medium">{match.compatibilityScore}% Compatível</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        <div className="text-center space-y-2 my-6">
          <div className="inline-flex items-center gap-2 bg-gray-200 text-gray-600 px-4 py-1 rounded-full text-xs">
            <Lock className="w-3 h-3" /> Chat Anónimo
          </div>
          <p className="text-xs text-gray-400">
            Podem partilhar contactos livremente escrevendo no chat. <br/>
            Esta conversa desaparecerá para sempre em 24h.
          </p>
        </div>
          ...
          if (msg.isSystemMessage) {
            ...
            return (
              <div key={msg.id} className="flex justify-center my-6 animate-fade-in">
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 text-gray-800 p-6 rounded-2xl text-sm border border-purple-100 max-w-[90%] md:max-w-[80%] shadow-sm relative overflow-hidden">
                   {/* Background Decor */}
                   <div className="absolute top-0 right-0 w-24 h-24 bg-purple-200 rounded-full mix-blend-multiply filter blur-2xl opacity-20 -translate-y-1/2 translate-x-1/2"></div>
                   ...
                   <div className="flex flex-col items-center gap-3 text-center relative z-10">
                      ...
                      <h4 className="font-bold text-purple-900 uppercase tracking-wider text-xs">Análise de Compatibilidade</h4>
                      
                      <div className="prose prose-sm prose-purple text-gray-700 leading-relaxed">
                        {/* We use dangerouslySetInnerHTML safely here or just react rendering since it might contain bold tags from our generator */}
                        {msg.text.split('**').map((part, i) => 
                            i % 2 === 1 ? <strong key={i} className="text-purple-900 font-bold block mt-2 mb-1">{part}</strong> : part
                        )}
                      </div>
                   </div>
                </div>
      ...
      <div className="bg-white p-4 border-t border-gray-200 safe-area-bottom">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            className="flex-1 border border-gray-300 rounded-full px-4 py-2 focus:ring-2 focus:ring-purple-500 outline-none transition-all"
          />
